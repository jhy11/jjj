...
def get_all_delivery(user_id):
    return order_product.objects.filter(product__shop_id__manager_id__user_id=user_id, order__type=constants.SHORTDELIVERY,  DeleteFlag='0')\
...
def get_delivery(user_id, status):
    return order_product.objects.filter(product__shop_id__manager_id__user_id=user_id, order__type=constants.SHORTDELIVERY, order__status=status, DeleteFlag='0')\
              .values('order__code', 'order__call', 'order__total_price', 'order__order_no', 'order__status', 'order__transport_no', 'order__date')
